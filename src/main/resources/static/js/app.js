...
async function getTable() {
    let temp = await fetch('http://localhost:8080/api/admin/table/')
    if (temp.ok) {
...
async function contentEditModal(id) {
    ...
    let ed_url2 = 'http://localhost:8080/api/admin/user/' + id
...
async function contentDeleteModal(id) {
    ...
    let del_url2 = 'http://localhost:8080/api/admin/user/' + id
...
del_form.addEventListener('submit', e => {
    e.preventDefault()
    let del_url = 'http://localhost:8080/api/admin/delete/' + id_del.value;
    let method = {
        method: 'DELETE',
        headers: {
            "Content-Type": "application/json"
